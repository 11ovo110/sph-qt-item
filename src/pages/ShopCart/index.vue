  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="good in cartInfoList" :key="good.id">
          ...
          <li class="cart-list-con5">
            <a @click="Minus(good)" class="mins">-</a>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllChecked" @change="checkedAll">
        <span>全选</span>
      </div>
      <div class="option">
        <a @click="removeAllGood">删除选中的商品</a>
...
export default {
  name: 'ShopCart',
  methods: {
    Minus: throttle(async function(good) {
      let { skuId } = good;
      try {
        if (good.skuNum > 1) {
          let skuNum = -1;
    ...
    async changeChecked(good) {
      let { skuId } = good;
      let isChecked = good.isChecked ? 0 : 1;
      try {
  ...
  computed: {
    ...
    total() {
      return this.cartInfoList.reduce((pre, cur) => cur.isChecked ? pre + cur.skuNum : pre, 0)
    },
    totalPrice() {
      return this.cartInfoList.reduce((pre, cur) => cur.isChecked ? pre + cur.skuNum * cur.skuPrice : pre, 0)
    },
    isAllChecked() {
      return this.cartInfoList.every(good => good.isChecked) && this.cartInfoList.length > 0
    }
  },
}
</script>
